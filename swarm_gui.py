...
class GUI():
    ...
    def update(self):
        for key in self.drones:
            R = self.rotation_matrix(self.drones[key]['orientation'])
            L = self.drones[key]['L']
            points = np.array([ [-L,0,0], [L,0,0], [0,-L,0], [0,L,0], [0,0,0], [0,0,0] ]).T
            points = np.dot(R,points)
            points[0,:] += self.drones[key]['position'][0]
            points[1,:] += self.drones[key]['position'][1]
            points[2,:] += self.drones[key]['position'][2]
